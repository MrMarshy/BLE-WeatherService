...
static void on_write(ble_cus_t * p_cus, ble_evt_t const * p_ble_evt)
{
    ...
    if (p_evt_write->handle == p_cus->custom_value_handles.value_handle)
    {
        ...
        {
            // Update/Read Temperature value
            //nrf_gpio_pin_clear(20); 
        }
        else if(*p_evt_write->data == 0x02)
        {
            // Update/Read Pressure Value
            nrf_gpio_pin_set(20); 
        }
        else
        {
...
/**@brief Function for adding the Custom Value characteristic to the Service.
 *
 * @param[in]   p_cus        Custom Service structure.
...
uint32_t ble_cus_init(ble_cus_t * p_cus, const ble_cus_init_t * p_cus_init)
{
    ...
    // Initialize with default temperature value
    ble_cus_custom_value_update(p_cus, UINT8_C(12));
